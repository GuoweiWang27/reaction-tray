...
test('level 3 resolves two 1:2 reactions and completes', async ({ page }) => {
  ...
  await expect(page.locator('.reaction-cue--precipitate')).toHaveCount(1)
...
test('level 3 exposes failure and supports a clean retry', async ({ page }) => {
  ...
  await expect(page.getByRole('status')).toContainText('本轮实验失败')
  await expect(page.getByText('本轮实验失败', { exact: true })).toBeVisible()
  await expect(page.getByRole('region', { name: '本轮实验失败' })).toBeVisible()
